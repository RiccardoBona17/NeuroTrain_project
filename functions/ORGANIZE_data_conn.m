%% ARRANGING DATA DEPENDING ON THE INPUT FILE (PREPROCESSED OR UN-PREPROCESSED)

function [AnatFiles,FuncFiles] = ORGANIZE_data_conn(bool_preproc,sel_subs)
...
% Selecting the type of data we need (to-be-preprocessed vs already-preprocessed)
if bool_preproc == 0 % no preprocessing

    for i_sub = sel_subs
        % Entering structural and functional data
        AnatFiles_final = [AnatFiles_final; cellstr(conn_dir(['wc0c' i_sub{:} '_ses-01_T1w.nii']))];
        FuncFiles_final = [FuncFiles_final; cellstr(conn_dir(['swau' i_sub{:} '_ses-01_task-rest_bold.nii']))];
    end
elseif bool_preproc == 1 % preprocessing needed
    for i_sub = sel_subs
        % Entering structural and functional data
        AnatFiles_temp = [AnatFiles_temp; cellstr(conn_dir([i_sub{:} '_ses-01_T1w.nii']))];
        FuncFiles_temp = [FuncFiles_temp; cellstr(conn_dir([i_sub{:} '_ses-01_task-rest_bold.nii']))];
    end
end
